...
# Most of the code taken directly from https://paperswithcode.com/method/he-initialization
# The only change is the new Kaiming log normal intialization function based on the Kaiming normal function from the original
...
def kaiming_log_normal_(tensor, a=0, mode='fan_in', nonlinearity='leaky_relu'):
    r"""Fills the input `Tensor` with values according to the method
    described in `Delving deep into rectifiers: Surpassing human-level
    performance on ImageNet classification` - He, K. et al. (2015), using a
    normal distribution. The resulting tensor will have values sampled from
    ...
    std = gain / math.sqrt(fan)
    with torch.no_grad():
        return tensor.log_normal_(0, std)
